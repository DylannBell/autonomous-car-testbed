...
class Button:
    def __init__(self, x, y, w, h, maps=[], cars=[], strategies=[]):
        ...
        #Initilaise UI button, this image will be a small size, since divided by 2
        self.UI = pygame.transform.scale(pygame.image.load("media/UI.png"),
                                             (int(self.w / 2), int(self.h / 2 )))
    ...
    def render(self, screen):
        ...
        if self.maps:
    #Add UI image to the top corner
            screen.blit(self.UI, (self.x - self.w /1, self.y - self.h / 4))
